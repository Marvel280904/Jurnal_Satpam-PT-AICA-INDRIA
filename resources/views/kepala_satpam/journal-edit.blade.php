    ...
    <form id="jurnalForm" action="{{ route('jurnal.update', $jurnal->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group-row">
                ...
                <select name="next_shift_user_id" id="nextShiftSelect" required>
                    ...
                    @foreach ($satpams as $satpam)
                        <option value="{{ $satpam->id }}"
                            {{ $jurnal->next_shift_user_id == $satpam->id ? 'selected' : '' }}>
                            {{ $satpam->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="tanggal"
                    value="{{ \Carbon\Carbon::parse($jurnal->tanggal)->format('Y-m-d') }}" required>
            </div>
        ...
        @foreach ($itemsYesNo as $key => $label)
            <div class="form-group">
                ...
                <div class="radio-group">
                    <label><input type="radio" name="is_{{ $key }}" value="1"
                            {{ $jurnal->{"is_{$key}"} == 1 ? 'checked' : '' }} required> Ya</label>
                    <label><input type="radio" name="is_{{ $key }}" value="0"
                            {{ $jurnal->{"is_{$key}"} == 0 ? 'checked' : '' }} required> Tidak</label>
                </div>
        ...
        @foreach ($itemsMasukKeluar as $key => $label)
            <div class="form-group">
                ...
                <div class="radio-group">
                    <label><input type="radio" name="is_{{ $key }}" value="1"
                            {{ $jurnal->{"is_{$key}"} === 1 ? 'checked' : '' }}> Masuk</label>
                    <label><input type="radio" name="is_{{ $key }}" value="0"
                            {{ $jurnal->{"is_{$key}"} === 0 ? 'checked' : '' }}> Keluar</label>
                </div>
            ...
            <ul id="fileList" class="file-preview-list">
                ...
                @foreach ($jurnal->uploads as $upload)
                    <li class="existing-file" data-id="{{ $upload->id }}">
                        ...
                        <button type="button" class="remove-btn remove-existing"
                            data-id="{{ $upload->id }}">x</button>
                    </li>
                @endforeach
            </ul>
        </div>
        ...
        @foreach ($itemsYesNo as $key => $label)
            (function() {
                ...
                function sync() {
                    if (yes.checked) {
                        ta.required = true;
                        if (ta.value.trim() === '') ta.setCustomValidity(
                            'Keterangan wajib diisi jika memilih Yes.');
            ...
            (function() {
                ...
                function sync() {
                    if (masuk.checked || keluar.checked) {
                        ta.required = true;
                        if (ta.value.trim() === '') ta.setCustomValidity(
                            'Keterangan wajib diisi jika memilih Masuk atau Keluar.');
                        else ta.setCustomValidity('');
                    } else {
                        ta.required = false;
                        ta.setCustomValidity('');
