...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form?.addEventListener('submit', function (e) {
        ...
        const ROUTE_CHECK = @json(route('guard.jadwal.check'));
        const ROUTE_STORE = @json(route('guard.jadwal.store'));
        const ROUTE_AFTER = @json(route('guard.data'));

        // Check duplicate, then store
        fetch(`${ROUTE_CHECK}?start_date=${encodeURIComponent(startDate.value)}&end_date=${encodeURIComponent(endDate.value)}`)
        .then(r => r.json())
        .then(d => {
            if (d.exists) {
            alert('Jadwal pada rentang tanggal tersebut sudah ada!');
